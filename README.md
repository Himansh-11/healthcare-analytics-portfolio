# Healthcare Analytics Portfolio

## Hospital Readmission Analysis Dashboard

Interactive Power BI dashboard analyzing readmission patterns across 2,497 U.S. hospitals using CMS data.

### 🎯 Key Findings
- **$4.2B** potential Medicare savings identified
- **116%** performance variation across hospitals
- Geographic disparities reveal state-level quality differences
- Heart failure represents the highest-cost readmission condition

### 📊 Dashboard Features
- **Page 1:** National overview with geographic analysis
- **Page 2:** Hospital-level comparison and correlation analysis
- **Page 3:** Financial impact and strategic recommendations

### 🛠️ Skills Demonstrated
- Power BI Development
- DAX (Data Analysis Expressions)
- Healthcare Analytics
- Data Modeling
- Financial Analysis

### 📁 Files
- `Healthcare_Dashboard.pbix` - Full Power BI dashboard
- `page1-overview.png` - Screenshot of national overview
- `page2-comparison.png` - Screenshot of hospital comparison
- `page3-financial.png` - Screenshot of financial impact

### 🌐 View Live Portfolio
[View Portfolio Website](https://Himansh-11.github.io/healthcare-analytics-portfolio)

### 📧 Contact
Himansh Rajak | himanshr1107@gmail.com | [LinkedIn](https://linkedin.com/in/himansh-rajak)
```

View the full project: [YOUR GITHUB PAGES LINK]

#HealthcareAnalytics #DataAnalytics #PowerBI #Portfolio
